...
const isInTargetRegion = (lat: number, lng: number): boolean => {
  ...
  return (
    // Africa
    (lat >= TARGET_REGIONS.AFRICA.lat.min && lat <= TARGET_REGIONS.AFRICA.lat.max &&
     normalizedLng >= TARGET_REGIONS.AFRICA.lng.min && normalizedLng <= TARGET_REGIONS.AFRICA.lng.max) ||
    // Asia
    (lat >= TARGET_REGIONS.ASIA.lat.min && lat <= TARGET_REGIONS.ASIA.lat.max &&
     normalizedLng >= TARGET_REGIONS.ASIA.lng.min && normalizedLng <= TARGET_REGIONS.ASIA.lng.max) ||
    // Middle East
    (lat >= TARGET_REGIONS.MIDDLE_EAST.lat.min && lat <= TARGET_REGIONS.MIDDLE_EAST.lat.max &&
     normalizedLng >= TARGET_REGIONS.MIDDLE_EAST.lng.min && normalizedLng <= TARGET_REGIONS.MIDDLE_EAST.lng.max)
  );
};
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      <Globe
        ...
        hexPolygonLabel={(d: any) => d.__showHexagon ? `
          <div>
        ...
        onHexPolygonHover={(polygon: any | null, prevPolygon: any | null) => 
          polygon?.__showHexagon ? handleCountryHover(polygon as CountryData | null, prevPolygon as CountryData | null) : null
        }
        rendererConfig={{
          antialias: true,
          alpha: true,
